...
import { Calculator, FlaskConical, Microscope, Stethoscope, Syringe } from "lucide-react";
...
export function ToolsHero({ heading, subtitle, compact = false, className = "" }: ToolsHeroProps) {
    ...
    useEffect(() => {
        const icons = [Calculator, FlaskConical, Microscope, Stethoscope, Syringe];
    ...
    return (
        <section className={`relative ${compact ? "h-[200px] md:h-[300px]" : "h-[300px] md:h-[500px]"} flex items-center overflow-hidden ${className}`}>
            ...
            <div className="absolute inset-0 w-full h-full">
                {/* Gradient Background - Tech/Medical feel */}
                    ...
                    {!compact && (
                        ...
                        >
                            <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-500/10 border border-emerald-500/20 backdrop-blur-sm">
                                ...
                                <span className="text-sm text-emerald-200">Precision Tools</span>
                            </div>
                            <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-teal-500/10 border border-teal-500/20 backdrop-blur-sm">
                                ...
                                <span className="text-sm text-teal-200">Clinical Calculators</span>
                            </div>
                            <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-cyan-500/10 border border-cyan-500/20 backdrop-blur-sm">
                                ...
                                <span className="text-sm text-cyan-200">Instant Results</span>
                            </div>
                        </motion.div>
                    )}
                </motion.div>
